...
function saveCell() {
    ...
    // Cập nhật hiển thị
    const cell = document.querySelector('[data-time="' + time + '"][data-day="' + day + '"]');
    if (cell) {
...
function saveData() {
    try {
        ...
        window.savedTimetableData = dataToSave;
        console.log('Dữ liệu đã được lưu vào bộ nhớ');
    } catch (error) {
...
function clearAll() {
    if (confirm('Bạn có chắc chắn muốn xóa toàn bộ thời gian biểu?')) {
        timetableData = {};
        window.savedTimetableData = null;
        generateTimetable();
